[] spawn {
    ...
    while{true} do
    {

        {
            ...
            //PAPABEAR sidechat ("BEFORE: "+(str PlayerCurrentZoneIndex)+" "+(str _currentPos)+" "+(str _currentMaxPoints)+" "+(str _currentOpforCount)+" "+(str _currentBluforCount)+" "+(str _currentPoints));
    ...
    while{true} do
    {
        if(PlayerCurrentZoneIndex != -1) then
        {
            ...
            if(_currentPoints > 0) then
            {
                ...
                _percent ctrlSetText "Controlled by BLUFOR ("+(str _currentPoints)+" / "+(str _currentMaxPoints)+")";
            ...
            else
            {
                ...
                _percent ctrlSetText "Controlled by OPFOR ("+(str (_currentPoints * -1))+" / "+(str _currentMaxPoints)+")";
